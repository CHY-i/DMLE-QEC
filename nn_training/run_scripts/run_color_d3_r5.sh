...
METHOD="${1:?usage: run_color_d3_r5.sh <si1000|ca|rl|aligned> <physical_gpu>}"
PHYSICAL_GPU="${2:?usage: run_color_d3_r5.sh <si1000|ca|rl|aligned> <physical_gpu>}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
DEM_ROOT="${REPO_ROOT}/nn_training/dems/color/d3_r5"
EVAL_ROOT="${EVAL_ROOT:-${PATCHDMLE_COLOR_DATA:-${REPO_ROOT}/dataset/color_code}/d3X/r05}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/nn_training/checkpoints_color_d3_r5}"
...
exec "${PYTHON}" nn_training/train_surface_dem.py \
  --dem-path "${DEM_PATH}" \
  --run-name "color_d3_r5_${METHOD}_${RUN_SUFFIX}" \
