...
METHOD="${1:?usage: run_surface_d3_r5.sh <si1000|ca|rl|aligned> <physical_gpu>}"
PHYSICAL_GPU="${2:?usage: run_surface_d3_r5.sh <si1000|ca|rl|aligned> <physical_gpu>}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
DEM_ROOT="${REPO_ROOT}/nn_training/dems/surface/d3_r5"
EVAL_ROOT="${EVAL_ROOT:-${PATCHDMLE_SYCAMORE_DATA:-${REPO_ROOT}/dataset/sycamore}/sample_00/d3_at_q6_5/X/r05}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/nn_training/checkpoints_surface_d3_r5}"
RUN_SUFFIX="${RUN_SUFFIX:-formal_1000000}"
...
exec "${PYTHON}" nn_training/train_surface_dem.py \
  --dem-path "${DEM_PATH}" \
  --run-name "surface_d3_r5_${METHOD}_${RUN_SUFFIX}" \
